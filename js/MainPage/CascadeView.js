...
var CascadeEnvironment = function (goldenContainer) {
  ...
  messageHandlers["combineAndRenderShapes"] = ([
    ...
    facelist.forEach((face, globalFaceIndex) => {
      let vertices = [],
        normals = [],
        triangles = [],
        uvs = [],
      ...
      model.name = `Model Face ${globalFaceIndex}`;
    ...
    this.fogDist = Math.max(
      this.fogDist,
      this.boundingBox.min.distanceTo(this.boundingBox.max) * 1.5
    );
  ...
  this.animate = function animatethis() {
    ...
    if (this.mainObject) {
      ...
      if (this.environment.controls.state < 0 && intersects.length > 0) {
        ...
        let newIndex = isLine
          ? intersects[0].object.getEdgeMetadataAtLineIndex(intersects[0].index)
              .localEdgeIndex
        ...
        if (
          ...
          if (this.highlightedObj) {
            if (this.highlightedObj.type === "LineSegments") {
              ...
              this.highlightedObj.material =
                this.highlightedObj.originalMaterial;
            }
          }
          this.highlightedObj = intersects[0].object;
          this.highlightedIndex = newIndex;

          if (!this.highlightedObj.originalMaterial && !isLine) {
            this.highlightedObj.originalMaterial = this.highlightedObj.material;
          }

          if (isLine) {
            this.highlightedObj.highlightEdgeAtLineIndex(intersects[0].index);
          } else {
            this.highlightedObj.material = new THREE.MeshBasicMaterial({
              color: 0xffff00, // 노란색
            });
          }
          this.environment.viewDirty = true;
        ...
        if (this.highlightedObj) {
          if (this.highlightedObj.type === "LineSegments") {
            ...
            this.highlightedObj.material = this.highlightedObj.originalMaterial;
          }
          this.environment.viewDirty = true;
        }
    ...
    //     let newIndex = intersects[0].faceIndex;
    //     if (
    //       this.highlightedObj != intersects[0].object ||
    //       this.highlightedIndex !== newIndex
    //     ) {
    ...
    //       if (!this.highlightedObj.originalMaterial) {
    //         this.highlightedObj.originalMaterial = this.highlightedObj.material;
    //       }
    ...
    //     let indexHelper = "Face Index: " + this.highlightedIndex;
    //     this.goldenContainer.getElement().get(0).title = indexHelper;
    //   } else {
    //     if (this.highlightedObj) {
    //       this.highlightedObj.material = this.highlightedObj.originalMaterial;
    ...
    //       ? intersects[0].object.getEdgeMetadataAtLineIndex(intersects[0].index)
    //           .localEdgeIndex
    //       : intersects[0].object.geometry.attributes.color.getX(
    //           intersects[0].face.a
    //         );
    //     if (
    //       this.highlightedObj != intersects[0].object ||
