...
let starterCode = `// Welcome to Cascade Studio!   Here are some useful functions:
...
// sceneShapes.push(externalShapes["model.stp"]);
...
function initialize(projectContent = null) {
  ...
  myLayout.registerComponent("codeEditor", function (container, state) {
    myLayout.on("initialised", () => {
      ...
      var extraLibs = [];
      let prefix = window.location.href.startsWith("https://zalo.github.io/")
        ? "/CascadeStudio/"
        : "";
      // opencascade.js Typescript Definitions...
      fetch(prefix + "node_modules/opencascade.js/dist/oc.d.ts")
        .then((response) => {
          response.text().then(function (text) {
            extraLibs.push({
              content: text,
              filePath:
                "file://" + prefix + "node_modules/opencascade.js/dist/oc.d.ts",
            });
          });
        })
      ...
      fetch(prefix + "node_modules/three/build/three.d.ts")
        .then((response) => {
          response.text().then(function (text) {
            extraLibs.push({
              content: text,
              filePath:
                "file://" + prefix + "node_modules/three/build/three.d.ts",
            });
          });
        })
        .catch((error) => console.log(error.message));
        ...
        .then((response) => {
          response.text().then(function (text) {
            extraLibs.push({
              content: text,
              filePath:
                "file://" + prefix + "js/StandardLibraryIntellisense.d.ts",
            });
      ...
      monacoEditor.evaluateCode = (saveToURL = false) => {
        ...
        messageHandlers["addCheckbox"]({ name: "Transparent?", default: true });
        messageHandlers["addSlider"]({
          name: "Opacity",
          default: 1,
          min: 0.01,
          ...
          window.history.replaceState(
            ...
            new URL(
              location.pathname +
                ...
                encode(newCode) +
                "&gui=" +
                encode(JSON.stringify(GUIState)),
              location.href
            ).href
          );
        }
  ...
  myLayout.registerComponent("console", function (container) {
    ...
    container.getElement().get(0).style.boxShadow =
    ...
    if (!realConsoleLog) {
      ...
      console.log = function (message) {
        ...
        newline.style.color = (alternatingColor = !alternatingColor)
          ? "LightGray"
          : "white";
        ...
        consoleContainer.parentElement.scrollTop =
          consoleContainer.parentElement.scrollHeight;
        realConsoleLog.apply(console, arguments);
      };
      // Call this console.log when triggered from the WASM
      messageHandlers["log"] = (payload) => {
        console.log(payload);
      };
  ...
  myLayout.updateSize(
    window.innerWidth,
    window.innerHeight - document.getElementById("topnav").offsetHeight
  );
  ...
  messageHandlers["addSlider"] = (payload) => {
    ...
    addGuiSeparator();
    const slider = gui.addInput(GUIState, payload.name, params);
  ...
  messageHandlers["addButton"] = (payload) => {
    addGuiSeparator();
    gui
      .addButton({ title: payload.name, label: payload.label })
