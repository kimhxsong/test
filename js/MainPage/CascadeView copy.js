...
var Environment = function (goldenContainer) {
  ...
  this.initEnvironment = function () {
    ...
    this.light3 = new THREE.AmbientLight(0x606060);
  ...
  this.onWindowResize = function () {
    ...
    this.camera.aspect =
      this.goldenContainer.width / this.goldenContainer.height;
    this.camera.updateProjectionMatrix();
    this.renderer.setSize(
      this.goldenContainer.width,
      this.goldenContainer.height
    );
...
var CascadeEnvironment = function (goldenContainer) {
  ...
  this.matcap = this.loader.load("./textures/dullFrontLitMetal.png", (tex) => {
  ...
  messageHandlers["combineAndRenderShapes"] = ([
    [facelist, edgelist],
    sceneOptions,
  ]) => {
    ...
    let globalFaceIndices = [],
      curGlobalFaceIndex = 0,
      faceVertices = 0,
      globalFaceMetadata = {};
    globalFaceMetadata[-1] = { start: -1, end: -1 };
    //
    let defaultFaceMaterial = new THREE.MeshPhongMaterial({
      color: new THREE.Color("rgb(255, 255, 255)"), // The color of the material. Default is white.
      vertexColors: false, // Whether or not to use vertex colors. Default is false.
      flatShading: true,
      side: THREE.DoubleSide,
    });

    defaultFaceMaterial.transparent = sceneOptions.transparent;
    defaultFaceMaterial.opacity = sceneOptions.opacity;
    // defaultFaceMaterial.side = sceneOptions.doubleSided ? THREE.DoubleSide : THREE.FrontSide;

    let highlightMaterial = defaultFaceMaterial.clone();
    highlightMaterial.color = new THREE.Color("rgb(255, 0, 0)");

    // let model = new THREE.Group();

    // let model = new THREE.Mesh(geometry, defaultFaceMaterial);
    // // model.castShadow = false;
    // model.globalFaceIndices = globalFaceIndices;
    // model.name = "Model";
    // model.globalFaceMetadata = globalFaceMetadata;
    let mainObject = this.mainObject;

    facelist.forEach((face) => {
      console.log(face.face_index);
      let faceMetadata = {};
      faceMetadata.localFaceIndex = face.face_index;
      faceMetadata.start = globalFaceIndices.length;
      ...
      // geometry.setAttribute( 'color', new THREE.Float32BufferAttribute( curColors, 3 ) );
      // geometry.setAttribute('uv', new THREE.Float32BufferAttribute(uvs, 2));
      // geometry.setAttribute('uv2', new THREE.Float32BufferAttribute(uvs, 2));
      // geometry.computeBoundingSphere();
      // geometry.computeBoundingBox();

      let mesh = new THREE.Mesh(geometry, defaultFaceMaterial);
      mesh.globalFaceIndices = globalFaceIndices;
      mesh.globalFaceMetadata = globalFaceMetadata;
      mesh.name = "Face " + face.face_index;
      // faceMetadata.material = defaultFaceMaterial;
      faceMetadata.mesh = mesh;
      faceMetadata.geometry = geometry;

      mainObject.add(mesh);
      // Use Vertex Color to label this face's indices for raycast picking

      faceMetadata.end = globalFaceIndices.length - 1;
      globalFaceMetadata[curGlobalFaceIndex] = faceMetadata;
      curGlobalFaceIndex++;
      globalFaceIndex++;
      vInd += face.vertex_coord.length / 3;

      mesh.highlightFaceAtIndex = function (triIndex) {
        let faceIndex =
          triIndex >= 0 ? this.globalFaceIndices[triIndex] : triIndex;
        if (faceIndex >= 0) {
          this.globalFaceMetadata[faceIndex].mesh.material = highlightMaterial;
          mesh.needsUpdate = true;
        }
      }.bind(mesh);

      mesh.getFaceMetadataAtIndex = function (faceIndex) {
        return this.globalFaceMetadata[this.globalFaceIndices[faceIndex]];
      }.bind(mesh);

      mesh.clearHighlights = function () {
        mesh.needsUpdate = true;
        return this.highlightFaceAtIndex(-1);
      }.bind(mesh);
    ...
    //
    //   model.highlightFaceAtIndex = function (triIndex) {
    //     if (triIndex < 0) {
    //       return
    //     }
    //     let faceIndex = this.globalFaceIndices[triIndex];
    //     this.globalFaceMetadata[faceIndex].mesh.material = highlightMaterial;
    //
    //     // if (triIndex >= 0) {
    //     //   let faceIndex = this.globalFaceIndices[triIndex];
    //     //   // let startIndex = this.globalFaceMetadata[faceIndex].start;
    //     //   // let endIndex = this.globalFaceMetadata[faceIndex].end;
    //     //
    //     //   // 특정 색상으로 하이라이트.
    //     //   for (let i = startIndex * 3; i <= endIndex * 3; i += 3) {
    //     //     this.colors[i] = 1.; // Red
    //     //     this.colors[i + 1] = 0.; // Green
    //     //     this.colors[i + 2] = 0.; // Blue
    //     //   }
    //   // }
    //
    ...
    // }.bind(model)
    //
    //   model.getFaceMetadataAtIndex = function (faceIndex) {
    //     return this.globalFaceMetadata[this.globalFaceIndices[faceIndex]];
    //   }.bind(model);
    //
    ...
    line.highlightEdgeAtLineIndex = function (lineIndex) {
      let edgeIndex =
        lineIndex >= 0 ? this.globalEdgeIndices[lineIndex] : lineIndex;
      ...
      // 특정 색상으로 하이라이트.
      for (let i = startIndex * 3; i <= endIndex * 3; i += 3) {
        this.lineColors[i] = 1; // Red
        this.lineColors[i + 1] = 0; // Green
        this.lineColors[i + 2] = 0; // Blue
      }
    ...
    this.fogDist = Math.max(
      this.fogDist,
      this.boundingBox.min.distanceTo(this.boundingBox.max) * 4
    );
    this.environment.scene.fog = new THREE.Fog(
  ...
  this.animate = function animatethis() {
    ...
    // if (this.mainObject) {
    //   // this.raycaster.znear = 0.1;
    //   // this.raycaster.zfar = 100;
    ...
    //     let isFace = intersects[0].object.type === "Mesh";
    //
    //     if (isFace) {
    //       let faceIndex = intersects[0].face.a; // Get the face index from the intersection
    //       let newIndex = intersects[0].object.getFaceMetadataAtIndex(faceIndex).localFaceIndex;
    //
    ...
    //
    //         this.highlightedObj.highlightFaceAtIndex(faceIndex);
    //         this.environment.viewDirty = true;
    //       }
    //     }
    //   } else {
    ...
    //   this.raycaster.setFromCamera(this.mouse, this.environment.camera);
    //   let intersects = this.raycaster.intersectObjects(this.mainObject.children,);
    //   // let intersects2 = this.raycaster.intersect(this.mainObject.children);
    //   if (this.environment.controls.state < 0 && intersects.length > 0) {
    ...
    //         (isFace? intersects[0].object.getFaceMetadataAtIndex(intersects[0].faceIndex).localFaceIndex : intersects[0].object.geometry.attributes.color.getX(intersects[0].face.a));
    //
    //     // let newIndex1 = isLine ? intersects[0].object.getEdgeMetadataAtLineIndex(intersects[0].index).localEdgeIndex : -1;
    //     // let newIndex = isFace ? intersects[0].object.getFaceMetadataAtIndex(intersects[0].faceIndex).localFaceIndex : intersects[0].object.geometry.attributes.color.getX(intersects[0].index);
    //
    //     if (this.highlightedObj !== intersects[0].object || this.highlightedIndex !== newIndex) {
    //       if (this.highlightedObj) {
    //         this.highlightedObj.material.color.setHex(this.highlightedObj.currentHex);
    ...
    //       if (isFace) {
    //         this.highlightedObj.highlightFaceAtIndex(intersects[0].faceIndex);
    ...
    if (this.mainObject) {
      ...
      let intersects = this.raycaster.intersectObjects(
        this.mainObject.children
      );

      if (this.environment.controls.state < 0 && intersects.length > 0) {
        let intersectedObject = intersects[0].object;
        let isLine = intersectedObject.type === "LineSegments";
        let isFace = intersectedObject.type === "Mesh";

        let newIndex;
        if (isLine) {
          newIndex = intersectedObject.getEdgeMetadataAtLineIndex(
            intersects[0].index
          ).localEdgeIndex;
        } else if (isFace) {
          newIndex = intersectedObject.getFaceMetadataAtIndex(
            intersects[0].faceIndex
          ).localFaceIndex;
        } else {
          // newIndex = intersectedObject.geometry.attributes.color.getX(intersects[0].face.a);
        }

        if (
          this.highlightedObj !== intersectedObject ||
          this.highlightedIndex !== newIndex
        ) {
          if (this.highlightedObj) {
            // this.highlightedObj.material.color.setHex(this.highlightedObj.currentHex);
            this.highlightedObj.material.color.setHex(0x00ffff);
    ...
    if (this.handles && this.handles.length > 0) {
      for (let i = 0; i < this.handles.length; i++) {
        this.environment.viewDirty =
          this.handles[i].dragging || this.environment.viewDirty;
      }
    }
    ...
    if (this.environment.viewDirty) {
      this.environment.renderer.render(
